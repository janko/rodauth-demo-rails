...
class JsonTest < ActionDispatch::IntegrationTest
  test "registration and login" do
    post "/login", params: { login: "user@example.com", password: "secret" }, as: :json
    ...
    post "/verify-account", params: { key: email_link[/key=(.+)/, 1], password: "secret" }, as: :json
    assert_equal 200, response.status
  ...
  test "change password" do
    ...
    post "/change-password", params: { password: "secret", "new-password": "new secret" }, as: :json,
